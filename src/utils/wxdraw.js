    ...
    if (!overlay) {
      for (var key in target) {
        if (source.hasOwnProperty(key)) {
          //如果是覆盖的话 只要源source 有那就覆盖掉。。。 不是那就沿用现在的这叫extend太绕了
          if (
        ...
        if (matrix.m === this.m && matrix.n == this.n) {
          this.matrixArray.forEach(function(everyM, index) {
            ...
            everyM.forEach(function(_everN, _index) {
              // 每一行的每一个
              Points[index][_index] =
                _everN + matrix.matrixArray[index][_index]; //最小城乘数因子
            });
          });
    ...
    },
    {
      ...
      value: function rotate(origin, angle) {
        if (origin) {
          ...
          var _temMatrix = rotateMatrix
            .multi(getChangeMatrix)
            .add(AtranslateMatrix);
...
var commonMethods = {
  ...
  setCommonstyle: function setCommonstyle(context) {
    ...
    if (
      ...
      gra = context.createLinearGradient.apply(
        context,
        toConsumableArray(this.getGradientOption(type).lg)
      );
    ...
    if (
      ...
      this.turnColorLock(true); //开启颜色锁
      gra = context.createCircularGradient.apply(
        context,
        toConsumableArray(this.getGradientOption(type).cg)
      );
      this.UnOption.gra.forEach(function(element) {
        // console.log(element);
        gra.addColorStop(element[0], element[1]);
...
var getCurvePoints = function getCurvePoints(
  pts,
  tension,
  isClosed,
  numOfSegments
...
Line.prototype = _extends(
  {
    ...
    getOriPoints: function getOriPoints() {
      ...
      this.posPoints.forEach(function(item) {
        _points.push([
          this.massCenter.x - item[0],
          this.massCenter.y - item[1]
        ]);
      }, this); //计算点位
...
Circle.prototype = _extends(
  {
    getOriPoints: function getOriPoints() {
      ...
      for (var i = 0; i <= 100; ++i) {
        ...
        points.push([
          this.Option.x + this.Option.r * Math.sin(sA),
          this.Option.y - this.Option.r * Math.cos(sA)
        ]);

        points2.push([
          this.Option.x +
            (this.Option.r + this.Option.lineWidth / 2) * Math.sin(sA),
          this.Option.y -
            (this.Option.r + this.Option.lineWidth / 2) * Math.cos(sA)
        ]);
      }
...
Rect.prototype = _extends(
  {
    ...
    _chengeCenter: function _chengeCenter(origin) {
      console.log(
        this.getPointTodraw(this.Option.x, this.Option.y, origin)[0][0],
        this.getPointTodraw(this.Option.x, this.Option.y, origin)[1][0]
      );
...
Cshape.prototype = _extends(
  {
    ...
    _pnpolyTest: function _pnpolyTest(x, y) {
      ...
      for (
        var i = 0, j = this._Points.length - 1;
...
var specialAtrr = {
  ...
  lineDash: {
    ...
    getIncre: function getIncre(source, tar, obj) {
      return [
        [-source[0][0] + tar[0][0], -source[0][1] + tar[0][1]],
        -source[1][1] + tar[1][1]
      ];
    }
  }
...
var AnimationFrag = function AnimationFrag(
  object,
  atrribute,
  exe,
  option,
  ...
  if (
    (typeof atrribute === 'undefined' ? 'undefined' : _typeof(atrribute)) ==
    'object'
  ) {
    //console.log('对象动画');
    this.genFlag = true;
      ...
      if (ele.name === name) {
        ...
        ele.thingsList.forEach(function(_ele) {
          if (scope !== 'no') {
            ...
            _ele.call.apply(
              _ele,
              [ele.scope].concat(toConsumableArray(_params))
            );
          }
...
AniFragWrap.prototype = {
  ...
  exeAnimate: function exeAnimate() {
    ...
    if (this.firstTime) {
      ...
      this.oriOption = util.extend(
        this.object.Shape.Option,
        this.object.Shape.Option
      );
      this.oriUnOption = util.extend(
        this.object.Shape.Option,
        this.object.Shape.UnOption
      ); // 记录最初的样式
...
Shape.prototype = {
  ...
  clone: function clone() {
    ...
    var _clone = new Shape(
      this.type,
      _extends({}, this.Shape.Option, this.Shape.UnOption, _spaAttr),
      this.strokeOrfill,
      this.draggable
    );
    return _clone;
...
Animation.prototype = {
  ...
  animationComplete: function animationComplete(who) {
    ...
    if (
      Object.keys(this.wraperAniCompleteOb).length ===
      Object.keys(this.animationFragStore).length
    ) {
  ...
  wraperAniComplete: function wraperAniComplete(afID, shaId, obj) {
    ...
    if (
      this.wraperAniCompleteOb[shaId].length ==
      this.animationFragStore[shaId].length
    ) {
